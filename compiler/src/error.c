...
void message_enqueue(message_context_t *ctx, int value, char *filename, char *error_msg, char *line_text, int line_no) {
    message_queue_t *queue;
    if (value == ERROR || value == ERROR_NOTE) {
        queue = &ctx->errors;
    ...
    if (!value && queue->count >= MAX_ERRORS) {
...
void error_print(message_queue_t *queue) {
    ...
    while (current) {
        if (current->type == ERROR) {
            ...
            int error_col = (pos) ? (pos - current->line_text) + 1 : 1;
            int error_len = strlen(current->error_msg);
            fprintf(stderr, "%s:%d: \033[1;31merror:\033[0m %s: '%s'\n",
                    current->filename, current->line_no, current->error_msg, current->error_msg);
            ...
            // Call note_print to print only the first note and then return
            note_print(current);
            break;  // Only print one note and return control back to error_print
        }
...
void warning_print(message_queue_t *queue) {
    ...
    while (current) {
        ...
        int error_col = (pos) ? (pos - current->line_text) + 1 : 1;
        int error_len = strlen(current->error_msg);
        fprintf(stderr, "%s:%d: \033[1;35mwarning:\033[0m %s: '%s'\n",
            current->filename, current->line_no, current->error_msg, current->error_msg);
        fprintf(stderr, "%*d | %s\n", width, current->line_no, current->line_text);
        fprintf(stderr, "%*s |\033[0;32m", width, "");
        for (int i = 0; i < error_col - 1; i++) fprintf(stderr, " ");
